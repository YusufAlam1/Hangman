...
public class HangmanGame 
{
    ...
    public static void playHangman(Word words, int wordIndex, Scanner scanN, Name userName) 
    {
        ...
        String guessedLettersForWord = ""; 
        displayHangman(hangman);

        while (lives != 0)
        {
            System.out.println("Please enter your guess:");
            char guess = scanN.next().charAt(0);

            if (guessedLettersForWord.indexOf(guess) != -1)
            {
            ...
            {
                guessedLettersForWord += guess; 
            }

            boolean found = false;
            String word = words.getWords()[wordIndex];
            char[] guessedLetters = words.getGuessedLetters()[wordIndex];

            for (int i = 0; i < word.length(); i++) 
            {
                if (Character.toLowerCase(guess) == Character.toLowerCase(word.charAt(i))) 
                {
                    words.revealLetter(guess, wordIndex);
                    found = true;
                    break;
                }
            }
            ...
            if (found) 
            {
                ...
                System.out.println("\nCorrect! The word is currently: " + wordDisplay.toString());
            }
            if (words.isGuessed(wordIndex))
            {
                System.out.println("Congratulations, Mr./Ms. " + userName.getLastName() + ", you have guessed the word correctly!");
                break;
            }
            if(!found)
            {
                ...
                if (bodyPartsAdded < maxBodyParts) 
                {
                    switch (mode)
                    {
                        case 1:
                    ...
                    addBodyPart(hangman, bodyPartsAdded);
                    System.out.println("\nSorry, the letter " + guess + " is not in the word. You have " + lives + " more attempt(s).");
            ...
            if (lives == 0)
            {
                System.out.println("Sorry, " + userName.getFirstName() + ", you lost! The word was: " + words.getWords()[wordIndex]);
            }
        }

    }

    public static void addBodyPart(char[][] hangman, int bodyPartIndex) {
    ...
    public static void displayHangman(char[][] hangman) 
    {
